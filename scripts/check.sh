...
! grep -q 'raw.githubusercontent.com/AbcCdx/ABX-Voice-Assist/main/' README.md
grep -q 'raw.githubusercontent.com/AbcCdx/ABX-Voice-Assist/v'"$app_version"'/' README.md
grep -q '^REF="${ABX_VOICE_ASSIST_REF:-\$SOURCE_COMMIT}"$' install.sh
grep -q '^EXPECTED_SOURCE_COMMIT="${ABX_VOICE_ASSIST_SOURCE_COMMIT:-\$SOURCE_COMMIT}"$' install.sh
