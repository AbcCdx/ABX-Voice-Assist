...
ABX Voice Assist is designed for local dictation.
...
- Successful transcripts, timing statistics, corrections, and preferences are
  stored under `~/Library/Application Support/ABX Voice Assist`.
...
ABX Voice Assist uses the network to download the speech model through
...
macOS Keychain (service `com.abc.abxvoiceassist.ai`), never in preferences
