...
REPOSITORY="${ABX_VOICE_ASSIST_REPOSITORY:-AbcCdx/ABX-Voice-Assist}"
...
RELEASE_URL="${ABX_VOICE_ASSIST_RELEASE_URL:-https://github.com/$REPOSITORY/releases/download/v$RELEASE_VERSION/ABX-Voice-Assist.zip}"
EXPECTED_SHA256="${ABX_VOICE_ASSIST_RELEASE_SHA256:-$RELEASE_SHA256}"
REF="${ABX_VOICE_ASSIST_REF:-$SOURCE_COMMIT}"
EXPECTED_SOURCE_COMMIT="${ABX_VOICE_ASSIST_SOURCE_COMMIT:-$SOURCE_COMMIT}"
APP_PATH="${ABX_VOICE_ASSIST_APP_PATH:-/Applications/ABX Voice Assist.app}"
BUILD_FROM_SOURCE="${ABX_VOICE_ASSIST_BUILD_FROM_SOURCE:-0}"
NO_OPEN="${ABX_VOICE_ASSIST_NO_OPEN:-0}"
AGENT_LABEL="com.abc.abxvoiceassist.agent"

say() {
    printf '\033[1;36mABX Voice Assist:\033[0m %s\n' "$*"
}

fail() {
    printf '\033[1;31mABX Voice Assist:\033[0m %s\n' "$*" >&2
    exit 1
}
...
verify_app() {
    local app="$1"
    local executable="$app/Contents/MacOS/ABXVoiceAssist"
    local bundle_id version minimum_system entitlements_file audio_input microphone

    [[ -x "$executable" ]] || fail "В архиве нет исполняемого файла ABX Voice Assist."
    bundle_id="$(plutil -extract CFBundleIdentifier raw -o - "$app/Contents/Info.plist")"
    [[ "$bundle_id" == "com.abc.abxvoiceassist" ]] || fail "Неверный идентификатор приложения: $bundle_id"
...
download_release() {
    local work_dir="$1"
    local archive="$work_dir/ABX-Voice-Assist.zip"
    local actual
    ...
    [[ -d "$work_dir/release/ABX Voice Assist.app" ]] || fail "В релизе нет ABX Voice Assist.app."
    ditto "$work_dir/release/ABX Voice Assist.app" "$work_dir/ABX Voice Assist.app"
}
...
build_from_source() {
    ...
    "$source_dir/scripts/build-app.sh" "$work_dir/ABX Voice Assist.app"
}
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/abxvoiceassist-install.XXXXXX")"
...
verify_app "$WORK_DIR/ABX Voice Assist.app"
say "Устанавливаю приложение в $APP_PATH..."

if [[ "$APP_PATH" == "/Applications/ABX Voice Assist.app" ]]; then
    /bin/launchctl bootout "gui/$UID/$AGENT_LABEL" >/dev/null 2>&1 || true
    /usr/bin/pkill -x ABXVoiceAssist >/dev/null 2>&1 || true
fi

INCOMING="$(dirname "$APP_PATH")/.ABX-Voice-Assist.install.$$"
BACKUP="$(dirname "$APP_PATH")/.ABX-Voice-Assist.previous.$$"
run_as_admin rm -rf "$INCOMING"
run_as_admin rm -rf "$BACKUP"
run_as_admin ditto "$WORK_DIR/ABX Voice Assist.app" "$INCOMING"
verify_app "$INCOMING"
...
if [[ "$NO_OPEN" == "1" ]]; then
    ...
    say "Готово. Открываю ABX Voice Assist..."
    open "$APP_PATH"
