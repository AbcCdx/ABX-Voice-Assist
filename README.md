...
4. Дождитесь статуса `Работает`, зажмите **правый Option** и говорите.
   Отпустите **правый Option**, чтобы завершить запись и вставить текст.
...
- **Правый Option** - диктовка работает только пока клавиша зажата. Левый
  Option не запускает запись.
...
## Диагностический архив диктовок

Приложение локально хранит последние 100 голосовых записей вместе с сырой
расшифровкой, финальным вставленным текстом, уверенностью модели и замерами
задержки. При появлении 101-й записи самая старая запись и её метаданные
удаляются. Архив нужен, чтобы подтверждённые жалобы на распознавание можно было
сверить с исходной речью, а не исправлять по догадке.

Файлы находятся в `~/Library/Application Support/ABX Voice Assist/DictationArchive`.
Они имеют приватные права доступа и никуда не отправляются автоматически.
Каталог содержит личный звук и текст, поэтому его не следует публиковать целиком.
