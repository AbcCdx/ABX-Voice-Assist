...
1. Получите ключ у любого OpenAI-совместимого провайдера (например, OpenCode
   Zen на [opencode.ai](https://opencode.ai/docs/zen/), OpenAI, OpenRouter).
...
3. При необходимости измените базовый URL и модель (по умолчанию —
   `https://opencode.ai/zen/v1` и `laguna-s-2.1-free`, самая быстрая из
   бесплатных моделей Zen на момент проверки), включите переключатель
   и нажмите `Сохранить`. Кнопка `Проверить подключение` проверяет ключ и адрес.
4. Быстрый переключатель также есть в меню менюбара: `Text` → `AI cleanup`.

Важно: бесплатные модели OpenCode Zen могут сохранять отправленный текст для
улучшения модели. Для личной диктовки выберите платную модель без хранения
данных или другого провайдера. Если сервер недоступен или ответил ошибкой,
вставляется исходный локальный текст — диктовка никогда не теряется.
